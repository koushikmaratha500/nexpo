...
export class SplitService {
  static calculate(
    ...
    if (total <= 0) throw new Error('Amount must be a positive number');
    if (participants.length === 0) throw new Error('Split participants are required');
    ...
    if (mode === 'CUSTOM_AMOUNT') {
      let sum = 0;
      const results = participants.map((participant) => {
        if (!participant.included) return { ...participant, computedAmount: 0 };
        ...
        return { ...participant, computedAmount: roundMoney(participant.shareAmount) };
      });
    ...
    const results = participants.map((participant) => {
      ...
      if (participant.sharePercent == null || participant.sharePercent < 0) {
        throw new Error('Custom split percents require sharePercent for included members');
      }
