...
export async function POST(req: NextRequest) {
  try {
    await checkRateLimit(req, `admin_reset_password_${getRequestIp(req)}`, RATE_LIMIT_PRESETS.adminResetPassword);
    return await AuthController.resetAdminPassword(req);
  } catch (error) {
